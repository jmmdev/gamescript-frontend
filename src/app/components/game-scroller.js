...
export default function GameScroller({externalFlags, scrollerGames, scrollerIndex, title, isHighlight}) {
    ...
    const GetScrollButtons = ({child}) => {
        ...
        result.push(
           <button key={0}
           className={`flex items-center justify-center w-[2.5%] text-white hover:text-[#dd202d] ${scrollerRef.current && lastIndexScrolled > 0 ? "visible" : "invisible"} pointer-coarse:invisible `} 
           onClick={() => {
                let index;
