        ...
        for (let [i, g] of genres.entries()) {
            res.push(
                <div style={{display: 'flex'}} key={`g${g._id}`}>
                    <Link className={styles['game-tag-link']} href={{pathname: `/genres/${g.slug}/page/${1}`}} as={`/genres/${g.slug}`}>
                        <p>{g.name}</p>
                    </Link>
        ...
        for (let [j, t] of themes.entries()) {
            res.push(
                <div style={{display: 'flex'}} key={`t${t._id}`}>
                    <Link className={styles['game-tag-link']} href={{pathname: `/themes/${t.slug}/page/${1}`}} as={`/themes/${t.slug}`}>
                        <p>{t.name}</p>
                    </Link>
