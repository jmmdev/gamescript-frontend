...
export default function Home() {
    ...
    useEffect(() => {
        async function getTheme() {
            try {
                const response = await fetch(`https://gamescript-backend.vercel.app/theme/${params.slug[0]}`,
                {
                    method: 'GET'
                });
    ...
    useEffect(() => {
        async function getThemeGames() {
            try {
                const response = await fetch(`https://gamescript-backend.vercel.app/gamesByTheme/${theme._id}/page/${params.slug[1]}`,
                {
                    method: 'GET'
                });
